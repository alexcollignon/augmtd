...
- **Actions follow intent + item work-plans (July 5, `docs/home-actions-plan.md` + `docs/home-actions-stage3-plan.md`):** every deep-dive leads with the natural action, not type-locked buttons. **Universal drafter:** `POST /api/compose/draft` (infers recipient — meeting→attendees, commitment→counterparty, awareness→sender; drafts in the user's voice; NEVER invents a recipient — unknown → empty `to` + name) + `POST /api/compose/send` (sends AS the user via connected mailbox, falls back to `sendCoworkerEmail`). A shared `ComposePanel` (reuses `ReplyEditor`) opens pre-filled. **Stage 2 — "What this takes"** (`lib/home/item-plan.ts` `generateItemPlan` + `item_plans` table, `20260705_item_plans.sql`): decomposes an item into 2–5 tasks, each graded **[System]** (draft/analyze/fetch/send — what we can actually do) or **[You]**, **instance-honest** (won't claim "fetch the deck" unless the deck is evidenced — unknown files/recipients become [You] steps). Cached + background pre-gen (top ~4 items on brief load); draft+send collapse to one "Draft & send". ⚠️ **Reasoning-tier gotcha:** planning on `bedrock_optimised` is Kimi (a reasoning model) — needs `max_tokens: 8000` + a `reasoning`-channel content fallback, else it emits EMPTY content (finish_reason=length) and every plan falls back to "Handle this" (build can't catch this; only a real AI call does — `scripts/test-item-plan.ts`). **North star (stage 3, planned):** the plan becomes a grounded, executable, editable workflow on the existing Studio/AgentOS engine (content flows fetch→draft→send, pauses for [You]/inputs, override any step, never sends without approval).
