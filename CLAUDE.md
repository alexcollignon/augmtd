...
- **Home brief — shared-context "assemble → reconcile → synthesize"** (`app/api/home/brief/route.ts`). **Two layers:** (1) the FACTS/counts/item-lists are recomputed **every load** (never cached; `classifyItem` at render time; dismissed/completed filtered out so they can't resurface); (2) the AI PROSE is cached in `profiles.home_brief` behind a **signature** (`sig`) + 3h TTL — it rewrites when the sig changes: **today's date** (daily re-contextualization), needs-reply/meeting/commitment counts, overdue, waiting-on, FYI, freshest-item + freshest-commitment timestamps. So it refreshes on: new mail/meeting/commitment, a new day, an action (Done/Dismiss/Send changes the counts), or the 3h safety timer — whichever first. **Pull, not push** (reflects on open/reload; your own actions are the instant optimistic exception).
  - **Layer 1 `lib/home/brief-context.ts`** `buildBriefContext` → a deterministic **entity-centric people map** (per-person meetings + commitments + emails + recorded meetings/transcripts, keyed by **email identity**; a name-token fallback only for legacy name-only commitments). No AI.
  - **Layer 3 `lib/home/synthesize-brief.ts`** `synthesizeBrief` → **ONE grounded AI pass** over the full reconciled per-person context that reasons about supersession / staleness / grouping and writes the whole brief in a first-person PA voice. **Grounded** via `[Rn]/[Wn]/[Fn]` id tags the model must echo (can't invent items); returns `droppedItemIds` (superseded/stale) applied to cards + counts so the grid can't contradict the prose. This **replaced** the old 4 blind silo AI calls AND **subsumed + retired** the deterministic `SCHEDULING`-regex meeting-supersession bandaid (reasoning is general now, not per-pattern). **DIRECTION (locked, `docs/brief-and-labeling-plan.md`):** identity over fuzzy strings; minimal deterministic Layer 2; reasoning in the synthesis; evaluate across users, not one inbox's screenshots. (⚠️ synthesis is on `dev` `5887d1b`, build-verified, pending review before merge to `main`.)
  - Neural-orb header + subtle date ("WEDNESDAY, JULY 2") in `components/home/home-view.tsx`.
