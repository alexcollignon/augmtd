...
**Coworker-chat `@`-mentions** (`components/workers/worker-mention-input.tsx` = the worker chat composer; `/api/workers/mentions` source): `@` → **Coworkers / Tasks / Documents** picker (ported from `/work`'s `ChatInputBar` — streaming in `worker-chat-tab` untouched; clicking Mention again / clicking away strips the dangling `@`). On send, `worker-chat-tab` posts `mentions[]`; the chat route resolves them in `buildMentionContext` (and the AgentOS bridge path appends the same context to the message): **@Coworker** → directive to build on that teammate's work (`find_team_work` scoped to them), **@Task** → injects the task's latest output, **@Document** → pins the doc's full content (same read path as `read_team_work`; also covers "drop the document here" — reference an existing doc, no re-upload). External-file upload is NOT built (the `@Document` pin covers the in-system case).
