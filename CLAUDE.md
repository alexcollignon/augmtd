...
- **Dev-server fix** (`package.json`) — recurring `.next` corruption = TWO dev servers racing on one `.next`. Added `predev` (frees port 3000, scoped) + pinned `-p 3000` + a `clean` script. **`npm run build` throws a spurious `/_document` PageNotFoundError if a dev server touches `.next` mid-build** — kill next processes + `rm -rf .next` first.
