...
- **Labeling reliability** (`lib/inbox/rules/write-back.ts` `writeBackLabel` now returns a **boolean**, never throws): the label-sweep + the sync-time fast-path only stamp `source_data.labeled=true` on **real success** — a transient Gmail rate-limit/token failure stays unmarked and retries, instead of silently recording a label that never landed. **`app/api/cron/label-sweep`** (every 2h, throttled) is the eventually-consistent backstop; the **sync fast-path** (fyi/noise, `lib/email-sync/sync-emails.ts`) stamps `labeled` on success so fresh mail labels within seconds.
...
### Home simplification — bundling deck (DIRECTION, being built July 14+)

See memory `project_home_simplification_clustering_fix.md`. The Home "what needs you" is too granular — it shreds broad contexts (a meeting's action items, N similar applicants) into flat atoms. Direction: roll up to the meaningful UNIT (meeting / thread / **initiative**), render each as a card with an **adaptive lead** (reasoned per item) + supporting anchors (the "5 W's + move": who · what/outcome · why-it-matters · where/initiative · whose-court · when · next-move) + one action, **expandable** to the atoms; present as a **focus + peek deck** (one focused, 2–3 peeked, advance as you clear). Bundling is PRESENTATION grouping, never re-classification — every atom stays reachable, precedence chains hold, a bundle surfaces its hottest atom. The "why it matters" field starts CONSERVATIVE (grounded facts only). The **in-motion status board** (to do → doing → waiting → done) should replace Timeline as the 2nd lens + be where "Run/Do it" lands. Layers: **L0 remove "Your projects" from Home (DONE — `home-view.tsx`; `InitiativeStrip` + handlers now dead code)** → L1 bundling data → L2 bundle card → L3 deck flow.
