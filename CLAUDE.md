...
- **Runtime injection (smart-auto)**: `lib/work/worker-skills-context.ts` → `buildSkillsBlock(client, agentId)` renders a `[SKILLS — reusable instructions for how to handle specific kinds of work…]` block; each skill tagged with its `when_to_use` hint so the worker picks the right one per task (no per-conversation picking). Injected in **both** run paths for parity: the AgentOS bridge (`buildWorkerRunContext` → covers chat + scheduled `agent` steps, the live prod path) and the native loop (`chat/route.ts`). Rides the existing `dependencies.user_context` channel — **no AgentOS/Python redeploy needed**. *(Gap: the inline `execute-step.ts` fallback for task `agent` steps does not inject skills — only matters if `WORKERS_USE_AGENTOS` is off.)*
- **Chat tools — reach the whole library on demand** (`lib/tools/worker-skills.ts`): assigned skills inject automatically; these cover unassigned ones. `list_skills` (full library, marks which are assigned) + `apply_skill(skill_name)` (pulls one skill's instructions by name, even unassigned, to follow for the current task). Wired into both paths exactly like the task tools: native loop (`chat/route.ts` — registered for workers + dispatch + a `[SKILLS]` prompt note), AgentOS internal route (`app/api/internal/agentos/tasks/route.ts` actions), and Python `@tool`s in `infra/agentos/tools_tasks.py` (added to `TASK_TOOLS`). **The AgentOS-path tools require an AgentOS redeploy** (Python baked into image); the native path is live on Vercel deploy.
