...
function fallbackVerdict(reason: string): WorkVerdict {
  return { work: 'none', component: 'message_only', executor: { kind: 'user' }, gate: null, reason };
}
...
function coerceVerdict(raw: unknown, roster: RosterEntry[]): WorkVerdict | null {
  ...
  let component = String(r.component || '').toLowerCase();
  if (!WORKS.has(work)) return null;
  // STRUCTURAL coherence — the model picks the WORK; when its component half drifts (or is
  // missing), the registry supplies the consistent mount. `none` is the only message_only work.
  const natural = componentForWork(work);
  if (!COMPONENT_KEYS.has(component)) component = natural ?? '';
  else if (natural && ((work !== 'none' && component === 'message_only') || (work === 'none' && component !== 'message_only'))) {
    component = natural;
  }
...
export async function judgeWork(client: SupabaseClient, userId: string, input: JudgeInput): Promise<WorkVerdict> {
  try {
    ...
    const sig = `${JUDGE_VERSION}:${activityAt}:${pool.length}:${pool[0]?.at ?? ''}`;
    ...
    if (input.kind === 'inbox' && threadMsgs.length) {
      ...
      if (st.lastMessageFromUser) {
        const v = fallbackVerdict('you have the last word on this thread — nothing owed until they reply');
    ...
    const res = await aiCall<Record<string, unknown>>({
      ...
      prompt:
        ...
        `JSON only: {"work":"…","component":"…","executor":{"kind":"coworker|user|system","name":"<team name if coworker>"},"options":[{"label":"…"}],"reason":"<one sentence>"}`,
    });
