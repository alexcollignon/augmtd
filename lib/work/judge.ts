  ...
  try {
    ...
    const res = await aiCall<Record<string, unknown>>({
      ...
      prompt:
        ...
        // The WEEKDAY is stated, never derived — "by Thursday" / "tomorrow" reasoning from a bare
        // ISO date made the model guess the day of week (and get it wrong: real mootness misfires).
        `TODAY is ${new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })} (${new Date().toISOString().slice(0, 10)}). The item's last activity was ${activityAt.slice(0, 10) || 'unknown'}.\n\n` +
