...
export default function GanttChart({ groups, today, onOpenGroup, emptyLine }: {
  ...
  // Anchor the window on NOW: a little past context, real future due dates, but bounded so a stray old
  // done-item (far past) or a lone far-future due date can't stretch the axis into mostly-empty space.
  const earliest = allDates.reduce(minS, allDates[0]);
  const latest = allDates.reduce(maxS, allDates[0]);
  ...
  return (
    <div ref={scrollRef} className="mt-4 overflow-x-auto rounded-xl border border-neutral-200/70">
          ...
          {datedGroups.map((g) => {
            ...
            return (
              <div key={g.id}>
                ...
                {/* Task rows — a dated EVENT per row. Wrapped in an animated clip so expand/collapse glides
                    (max-height = row count × ROW_H). */}
                    ...
                    <div key={idx} className="relative flex items-center border-b border-neutral-50 last:border-b-0 group hover:bg-neutral-50/50 transition-colors" style={{ height: ROW_H }}>
                      <div className="sticky left-0 z-20 bg-white group-hover:bg-[#fafafa] h-full flex items-center gap-2 pl-8 pr-3 border-r border-neutral-200/60" style={{ width: LABEL_W }}>
                        ...
                        {it.href ? (
                          ...
                          <button onClick={() => onOpenGroup?.(g.id, 'work')} className="min-w-0 flex-1 flex items-baseline gap-1.5 text-left group/l" disabled={!onOpenGroup}>
                            <span className="text-[12px] text-neutral-700 truncate min-w-0 flex-1 group-hover/l:text-indigo-600 transition-colors">{it.title}</span>
                            {who && <span className="text-[10.5px] text-neutral-400 truncate flex-shrink-0 max-w-[116px]">{who}</span>}
                          </button>
                        )}
