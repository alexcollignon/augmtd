...
export default function UpcomingMeetingCard({ event, botState, onScheduled, onCancelled }: UpcomingMeetingCardProps) {
  ...
  const attendeeStr = event.attendees.length > 0
    ? event.attendees.slice(0, 2).map((a) => a.name?.split(' ')[0] || a.email?.split('@')[0]).join(', ') + (event.attendees.length > 2 ? ` +${event.attendees.length - 2}` : '')
    : null;

  return (
    <Link href={`/meetings/${event.id}`}>
      <div className="px-3 py-2.5 bg-white border-b border-neutral-100 hover:bg-neutral-50 transition-colors cursor-pointer">
        {/* Title + status */}
        <div className="flex items-center justify-between gap-2">
          <h3 className="text-[12px] font-semibold text-neutral-900 truncate leading-snug">
            {event.title}
          </h3>
          {statusBadge && (
            <span className={`flex-shrink-0 text-[10px] font-medium px-1.5 py-0.5 ${statusBadge.className}`}>
              {statusBadge.label}
            </span>
          )}
        </div>

        {/* Time + attendees — single truncated line */}
        <p className="text-[11px] text-neutral-400 mt-0.5 truncate">
          {primary} · {duration}min{attendeeStr ? ` · ${attendeeStr}` : ''}
        </p>

        {/* Bot state row — Google Meet only */}
        {isMeet && (
        ...
        {linkedWork && totalChips > 0 && (
          <div className="flex flex-wrap gap-1 mt-1.5">
            {linkedWork.processes.slice(0, 1).map((p) => (
