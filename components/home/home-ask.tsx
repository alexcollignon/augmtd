...
export default function HomeAsk({ briefing, clearedIds, onBriefNavigate, suggestions }: {
  ...
  return (
    <section className="flex flex-col flex-1 min-h-0 w-full">
      {/* Spacer — the chat block owns the bottom; everything above breathes. */}
      <div className="flex-1" />
      <div className="sticky bottom-0 z-20 pt-4 pb-5 bg-gradient-to-t from-[#fbfbfd] via-[#fbfbfd] to-transparent">
        <div
          onMouseEnter={() => setHovered(true)}
          onMouseLeave={() => setHovered(false)}
          className={`transition-all duration-300 ease-out ${expanded
            ? 'rounded-2xl border border-neutral-200 bg-white shadow-[0_12px_48px_-12px_rgba(23,23,23,0.18)] p-4'
            : 'rounded-2xl'}`}
        >
          {/* Header — only in the open card. */}
          {expanded && hasThread && (
          ...
          {/* The brief — a tight 2-line preview on hover, full inside the open card. Directly above the
              composer, never orphaned at the top of the page. */}
          {briefing && (
            <div
              className="relative overflow-hidden transition-all duration-300 ease-out"
              style={{ maxHeight: expanded ? 480 : hovered ? 64 : 0, opacity: expanded || hovered ? 1 : 0 }}
            >
              <div className={expanded ? 'max-h-[460px] overflow-y-auto [scrollbar-width:thin] pr-1' : ''}>
                <BriefingBlock briefing={briefing} clearedIds={clearedIds} onNavigate={onBriefNavigate} flat />
              </div>
              {!expanded && <div className="pointer-events-none absolute inset-x-0 bottom-0 h-5 bg-gradient-to-t from-[#fbfbfd] to-transparent" />}
            </div>
          )}

          {/* The conversation — inside the card, its own scroll. */}
          {hasThread && (
            <div className="space-y-4 mt-4 max-h-[38vh] overflow-y-auto [scrollbar-width:thin] pr-1">
              {turns.map((t, i) => (
                t.role === 'user' ? (
          ...
          {/* Chips + composer — always present; the card grows around them. */}
          {!hasThread && suggestions.length > 0 && (
            <div className={`flex flex-wrap gap-1.5 mb-2.5 ${expanded || briefing ? 'mt-3' : ''}`}>
              {suggestions.map((s) => (
          ...
          <div className={`flex items-center gap-2 rounded-2xl border px-3.5 py-2.5 transition-all duration-200 ${expanded
            ? 'border-neutral-200 bg-neutral-50/60 focus-within:border-indigo-300 mt-1'
