...
export default function InboxTopBar({ preparedItems, todayMeetings, onSelectItem }: InboxTopBarProps) {
  const hasPrepared = preparedItems.length > 0;
  const hasMeetings = todayMeetings.length > 0;

  if (!hasPrepared && !hasMeetings) return null;
        ...
        <div className="flex items-center gap-2.5 flex-shrink-0">
          ...
          <div className="flex items-center gap-1.5">
            ...
            {todayMeetings.length > 5 && (
              <span className="text-[11px] text-neutral-400">+{todayMeetings.length - 5}</span>
            )}
          </div>
        </div>
      )}
