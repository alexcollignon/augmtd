...
export default function WorkDetailInline({ item, onItemConfirmed, onRefreshMeetings, pendingReplyDraft, onReplySent, replyBody, onReplyBodyChange, onReplyOpenChange, onOpenWorkflowPanel, folderSections }: WorkDetailInlineProps) {
  ...
  // Reset thread state on item change. (Per-message expand / fold / recipient state now lives
  // inside <ThreadMessages/>, which is keyed by item id so it remounts fresh per item.)
  useEffect(() => {
    ...
    <div className="flex flex-col h-full bg-neutral-100">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
        ...
        {/* Conversation thread stack — live from emails table, oldest first, latest expanded.
            Rendered by the SHARED <ThreadMessages/> (same component the Home item-detail uses) so
            the two surfaces render pixel-identically. Keyed by item id so its per-message expand /
            fold / recipient state resets fresh per item. */}
        <ThreadMessages
          key={item.id}
          messages={threadEmails}
          fallback={sourceData as any}
        />
