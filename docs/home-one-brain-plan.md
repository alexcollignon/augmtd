...
computations — "In motion" and "Projects/suggestions" diverge (Volta is a suggested project but never shows in
...
- **"In motion"** = the active **initiatives** — each shown by **state**, NOT a re-listing of its actions and
  ...
  - awareness (`nothing needed · last meeting Jul 10`, e.g. Volta) → **muted, sorted last**
...
Home In-motion and the Projects lens read it. Fix clustering once → both benefit → Volta appears everywhere by
...
- Smoke: Home set == Projects set (no more "in one not the other"); Volta present in both.
...
- Smoke: every action appears in the list; In-motion shows Volta (awareness) + Jordan (action-needed).
...
4. Calendar + outbound folded into the one source (Volta-class initiatives surface everywhere).
