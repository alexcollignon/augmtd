...
export async function GET(request: NextRequest) {
  try {
    ...
    import('@/lib/microsoft/outlook').then(async ({ fetchUnreadEmails, parseOutlookMessage }) => {
      ...
      try {
        ...
        const onTokenRefresh = async (newTokens: { accessToken: string; refreshToken: string; expiresOn: string }) => {
          ...
          console.log(`✓ Updated refreshed tokens during initial sync`);
        };

        const messages = await fetchUnreadEmails(encryptedTokens, 10, 7, onTokenRefresh);
        console.log(`Fetched ${messages.length} Outlook emails for initial sync`);
