...
export async function POST(
  ...
  try {
    ...
    if (item.is_read === false) return NextResponse.json({ ok: true }); // already unread
    ...
    if (connection.provider === 'gmail') {
      ...
      if (gmailId) {
        try {
          ...
          console.error('[mark-unread] Gmail modify failed:', err);
        }
      }
    } else {
      ...
      if (outlookId) {
        try {
          ...
          await graphClient.api(`/me/messages/${outlookId}`).patch({ isRead: false });
        } catch (err) {
          console.error('[mark-unread] Outlook patch failed:', err);
        }
      }
    }

    await supabase.from('inbox_items').update({ is_read: false }).eq('id', id).eq('user_id', user.id);
