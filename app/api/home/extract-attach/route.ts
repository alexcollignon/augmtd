...
export async function POST(request: NextRequest) {
  try {
    ...
    const attachments: Array<{ name: string; text: string | null }> = [];
    for (const f of files) {
      ...
      const mime = f.type && f.type !== 'application/octet-stream'
        ? f.type
        : MIME_BY_EXT[f.name.split('.').pop()?.toLowerCase() ?? ''] ?? f.type;
      try {
        const text = await extractTextFromAttachment(
          Buffer.from(await f.arrayBuffer()), mime as AttachmentMimeType, f.name);
