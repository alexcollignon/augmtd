...
// Body: { objective, kind?, answers: [{question, answer}], samples? }
// → { draft: { name, when_to_use, content, kind } }
...
export async function POST(request: NextRequest) {
  ...
  const { objective, kind, answers, samples } = (await request.json()) as {
    objective?: string;
    kind?: string;
  ...
  const samplesBlock = samples?.trim()
...
What it should capture: ${objective.trim()}
...
- name: short and clear, max ~6 words (e.g. "My LinkedIn voice", "Acme business context").
  ...
  try {
    ...
    return NextResponse.json({
      draft: {
        ...
        content,
        kind: ['voice', 'domain', 'audience', 'method'].includes(kind ?? '') ? kind : null,
      },
    });
  } catch (err) {
