...
async function executeChatTool(
  ...
  switch (name) {
    ...
    case 'create_task': {
      ...
      const raw = await executeCreateTask(
        description, ctx.agentId, ctx.userId, ctx.supabase, ctx.adminClient, skillNames, input.trigger_doors,
        // THE INPUTS TRAY (relay canvas W2) — spoken document names + the run-time material door.
        Array.isArray(input.input_doc_names) ? input.input_doc_names : undefined,
        typeof input.input_accept_material === 'boolean' ? input.input_accept_material : undefined,
        // THE THROTTLE (relay canvas W3b) — a stated pace; the executor clamps and says so.
        typeof input.daily_run_limit === 'number' ? input.daily_run_limit : undefined,
      );
    ...
    case 'update_task': {
      ...
      const result = await executeUpdateTask(taskId, {
        ...
        // THE EVENT DOORS (relay canvas W1) — additive verbs; the executor sanitises.
        ...(Array.isArray(input.add_trigger_doors) ? { add_trigger_doors: input.add_trigger_doors } : {}),
        ...(Array.isArray(input.remove_trigger_doors) ? { remove_trigger_doors: input.remove_trigger_doors as string[] } : {}),
        // THE INPUTS TRAY (relay canvas W2) — additive verbs; the executor resolves and stores.
        ...(Array.isArray(input.add_input_docs) ? { add_input_docs: input.add_input_docs } : {}),
        ...(Array.isArray(input.remove_input_docs) ? { remove_input_docs: input.remove_input_docs as string[] } : {}),
        ...(typeof input.input_accept_material === 'boolean' ? { input_accept_material: input.input_accept_material } : {}),
        // THE THROTTLE (relay canvas W3b) — its own store; the executor clamps and speaks it.
        ...(typeof input.daily_run_limit === 'number' ? { daily_run_limit: input.daily_run_limit } : {}),
    ...
    // no rules of its own — it calls `answerInputStation`, the same core the resume door calls.
    case 'supply_run_input': {
      const result = await executeSupplyRunInput(
        {
          run_id: typeof input.run_id === 'string' ? input.run_id : undefined,
          text: typeof input.text === 'string' ? input.text : undefined,
          kb_file_name: typeof input.kb_file_name === 'string' ? input.kb_file_name : undefined,
          pin: input.pin === true,
        },
        ctx.userId, ctx.adminClient,
      );
      return { result, summary: 'Material supplied' };
    }
