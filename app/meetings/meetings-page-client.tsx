...
export default function MeetingsPageClient({ userEmail }: { userEmail: string }) {
  ...
  const dayMeetings = upcoming.filter((e) => new Date(e.start_time).toDateString() === selectedDateStr);
  ...
  return (
    <div className="flex h-full">
      ...
      {/* ── Right column ── */}
      <div className="w-[260px] flex-shrink-0 border-l border-neutral-100 flex flex-col overflow-y-auto bg-white">
        {/* Calendar */}
        ...
        <div className="flex-1 px-3 py-3 overflow-y-auto">
          ...
          {loading ? (
            <div className="space-y-1.5">
              {[1, 2].map((i) => <div key={i} className="h-14 bg-neutral-100 animate-pulse rounded" />)}
            </div>
          ) : dayMeetings.length === 0 ? (
            <p className="text-[12px] text-neutral-400 italic px-1 py-2">No meetings this day.</p>
          ) : (
            <div className="space-y-1.5">
